...
The hybrid neural baseline uses 300-dimensional token embeddings, a single bidirectional LSTM layer with hidden size 256, dropout 0.35, batch size 32, and AdamW optimization with learning rate 0.001. The model is trained for up to 12 epochs with early stopping on validation F1. The CRF output layer preserves global transition constraints during decoding.

\paragraph{BERT Token Classification.}
The transformer baseline fine-tunes \nolinkurl{bert-base-cased} \cite{devlin2019bert} with maximum sequence length 128, batch size 16, learning rate \(3 \times 10^{-5}\), weight decay 0.01, warmup ratio 0.1, gradient accumulation, mixed-precision training, and early stopping patience 2. The strongest full-data run clearly outperformed the non-transformer baselines.
